...
export const declinedController = {
    ...
    deletedeclinedProperty: async (req: Request, res: Response) => {    
        const declinedPropertyId = req.params.declinedPropertyId;
          try {
             const deletefavortieProperty = await prisma.favorite.delete({
                 where: {
                     id: declinedPropertyId
                 }
             })
             return res.status(StatusCodes.OK).json({
              message: "Favorite properties deleted successfully.",
        ...
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
        message: "Failed to delete Favorite properties.",
      });
     }
