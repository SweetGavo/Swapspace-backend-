...
const propertyController = {
  ...
  getAllProperty: async (req: Request, res: Response): Promise<Response> => {
    try {
     ...
     const {for_Rent,for_Sale,Short_let,price_Range,area_Range,payment_frequency,property_type,specification,bedroom,bathroom,pets,renovation,views,proximity,add} =req.query
      if (for_Rent) filters.for_Rent = for_Rent;
      if (for_Sale) filters.for_Sale = for_Sale;
      if (Short_let) filters.Short_let = Short_let;
      // if (price_Range) {
      //   const productarr = [for_Rent, for_Sale, Short_let, price_Range, area_Range, payment_frequency, property_type, specification, bedroom, bathroom, pets, renovation, views, proximity, add];
      //   productarr.filter(item => item == price_Range)
      // }
      ...
      if (property_type) filters.property_type = ''
