...
const propertyController = {
  ...
  getAllProperty: async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
      ...
      if (short_let) {
        filters.short_let = for_rent
      }
      if (area_Range) {
        filters.area_Range = for_rent
      }
      if (payment_frequency) {
        filters.payment_frequency = for_rent
      }
      if (property_type) {
        filters.property_type = for_rent
      }
      if (specification) {
        filters.specification = for_rent
      }
      if (bedrooms) {
    ...
    const colation = await prisma.property.fin
